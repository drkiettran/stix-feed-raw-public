...
### CI tooling: a decision, not an assumption

This brief deliberately does not name a CI tool. Databricks Asset Bundles are runner-agnostic — the pipeline steps (`bundle validate`, `bundle deploy`) are identical on any platform — so the tool choice is an **S1 ADR decision**, made once and recorded with its rationale. What actually differs between candidates is how CI authenticates to Databricks, which is also the epic's riskiest dependency:

| Candidate | Auth to Databricks | Notes |
|---|---|---|
| GitHub Actions | OIDC federation (no stored secrets) | Best-documented DABs path; SaaS runners need network access to the workspace |
| Azure DevOps | Workload identity federation | Strongest if already licensed org-wide |
| Jenkins / CloudBees | Stored SP OAuth secret + rotation | Reintroduces long-lived credentials; self-hosted runners help with network-restricted workspaces |
| GitLab CI | OIDC (custom claim config) | Viable if GitLab is the org standard |

Selection criteria for the ADR, in priority order: (1) what the org already licenses and security has approved, (2) support for secretless OIDC auth, (3) runner network reachability to the workspace, (4) team familiarity. The S2/S4 templates will be built on whichever tool the ADR selects — this is why S1 blocks them.
...
Conservative plan: ~2 sprints with two engineers (~3.5 elapsed weeks), including ADR review cycles, CI auth debugging time, the external IAM dependency, and pilot-team scheduling. Relative sizing — calibrate to team velocity.
...
Alignment this sprint (ADR + refinement), build next sprint. The failure paths are explicit — objections today revise the brief; ADR revision cycles stay inside the sprint.

![Path to execution](diagrams/06-path-to-execution.svg)
